...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| group_id | string | 是 | 素材组 ID（用户完成真人认证后获取） |
...
2. **真人认证前置**：用户必须先完成真人认证并获取 GroupId，才能上传素材
3. **Project 隔离**：同一用户的素材和推理接入点必须在同一项目中
4. **面部一致性**：上传的素材将与真人认证基准图像进行面部特征一致性比对，非同一个人物或多个人脸无法入库
5. **异步处理**：CreateAsset 为异步接口，上传后需轮询 GetAsset 确认状态
6. **URL 有效期**：GetAsset 返回的素材 URL 有效期为 12 小时
7. **素材要求**：
   - 全身参考图：竖版，人物全身正面图片
   - 人脸特写图：竖版，人物正面无表情特写，肩部以上，面部占画面 2/3 左右
8. **视频生成指代**：在 Seedance Prompt 中需使用"图片1"、"视频1"等序号指代素材，不可直接使用 Asset ID
